<?php
  ...
  if ((isset($_GET['sid']) && $_GET['sid'] !== "") && (isset($_GET['pid']) && $_GET['pid'] !== "")) { 
    ...
    if ($pid == 0) { //Home-Feed
      ...
      $sql = "SELECT * FROM `scala_status` WHERE `uid` IN (".$fids.") AND `id` < '".$new_sid."' ORDER BY `time` DESC LIMIT 25";
      ...
      if ($query && mysql_num_rows($query) > 0) {  
        ...
        echo "<div><br />Keine weiteren Meldungen</div>\n";
      }
    } else { //Profil-Feed
      ...
      $sql = "SELECT * FROM `scala_status` WHERE ((`uid` = '".$pid."') OR (`rec_id` = '".$pid."')) AND `id` < '".$new_sid."' ORDER BY `time` DESC LIMIT 25";
      $query = mysql_query($sql);
      ...
      if ($query && mysql_num_rows($query) > 0) {      
        ...
        echo "<div><br />Keine weiteren Beitr&auml;ge</div>";
      }
    }
  } else {
